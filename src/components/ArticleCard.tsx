...
export default function ArticleCard({ article, className }: ArticleCardProps) {
  return (
    <Link href={`/articles/${article.slug}`} className={`group block cursor-pointer${className ? ` ${className}` : ''}`}>
      <article className="plant-card bg-white rounded-3xl border border-[#E2EFE7] overflow-hidden h-full flex flex-col">
        ...
        <div className="p-6 flex flex-col flex-1">
          ...
          <h2 
            className="font-bold text-xl text-[#0F172A] group-hover:text-[#15803D] transition-colors duration-200 leading-tight mb-3"
          ...
          <div className="flex items-center text-sm font-bold text-[#15803D] group-hover:gap-2 gap-1 transition-all duration-200 mt-auto">
